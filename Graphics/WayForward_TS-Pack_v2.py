            ...
            for ty in range(2):
                for tx in range(2):
                    ...
                    if Use256Colors:
                        ...
                        tile_pixels_store = tile_pixels
                        tile_palette = 0
                    else:
                        # 16-цветный режим: определяем суб-палитру из пиксельных индексов
                        # В PNG от Extract-скрипта индексы уже содержат номер суб-палитры:
                        # pixel = (raw_nibble & 0x0F) + (palette_row << 4)
                        # Определяем palette_row по первому ненулевому пикселю
                        palette_row = 0
                        for p in tile_pixels:
                            if p != 0:  # Индекс 0 = прозрачность (общая для всех суб-палитр)
                                palette_row = p >> 4
                                break
                        
                        # Извлекаем только низкие нибблы для хранения
                        tile_pixels_store = []
                        for p in tile_pixels:
                            if p == 0:
